...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            test = Test.objects.get(id="test_vision_zero_bot.py")
        except:
            test = Test(
                id="test_vision_zero_bot.py"
            )
            test.save()

        try:
            r = requests.get("https://mastodon.social/api/v1/accounts/110221970039225180/statuses")
            ...
            if time_since_last_post.seconds < 60 * 60 * 72:
                ...
                new_log.save()
                send_mail(
                    "Test Failure: test_vision_zero_bot",
                    f"{log}",
